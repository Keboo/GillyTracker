...
2. Ensure the GillyTracker is running at the configured base URL
...
```bash
dotnet run --project GillyTracker.AppHost
```
...
- name: Install Playwright Browsers
  run: pwsh GillyTracker.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- name: Run UI Tests
  env:
    TEST_BASE_URL: https://staging.example.com
  run: dotnet test GillyTracker.UITests/GillyTracker.UITests.csproj
```
...
- task: PowerShell@2
  ...
  inputs:
    targetType: 'inline'
    script: |
      pwsh GillyTracker.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- task: DotNetCoreCLI@2
  displayName: 'Run UI Tests'
  inputs:
    command: 'test'
    projects: 'GillyTracker.UITests/GillyTracker.UITests.csproj'
  env:
